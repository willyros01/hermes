...
Device retest showed lifecycle triggers could overlap after reconciliation and return an attempted Failed row to Sending. The expanded candidate places reconciliation and flush under one application-owned serialized cycle and keeps attempted disallowed rows Failed. Stage timeouts are transport safety boundaries, not orientation/render timing logic. The service-worker cache revision deterministically installs the same-version code; no reload synchronization or lifecycle listener is added.
...
## FIDUNIO 1.1.22 deterministic entry and projection correction

The per-conversation composer map may retain draft text, focus, selection and textarea height only. It may not retain scroll position across route changes. Before an owned background projection, `render()` captures the currently mounted chat viewport and hands it to that one same-conversation render. Near-bottom remains bottom as new rows arrive; a deliberately scrolled-up view remains stable. If no same-conversation viewport exists—notification entry, Messages-list entry or another-conversation selection—the structural chat renderer opens at the latest message.

The direct snapshot owner updates application message state and immediately requests the central background projection before cache persistence and server Read recovery. Known immutable e2ee:3 IDs reuse already authenticated plaintext; this avoids repeated history-wide E2EE/Firebase work without creating another message store or decrypt owner. Cold notification selection remains within the activation mutex and a pending route is finalized only after the exact target composer is mounted.
