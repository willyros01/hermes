...
import {subscribeCloudGroupMessages,updateCloudGroupReceipt,subscribeCloudGroupReceipts} from "./firebase.js";
...
function aggregateReceipt(row,receipts,myUid){
  if(row.senderUid!==myUid)return row.state||"sent";
  const recipients=(row.recipientUids||[]).filter(uid=>uid!==myUid);
...
export function subscribeAccountGroupConversation(groupId,{onRows,onError,isOpen=()=>false}={}){
  ...
  let rawRows=[],closed=false,delivery=Promise.resolve();
  const emit=async()=>{
    const out=[];
    for(const row of rawRows){
      ...
      out.push({id:row.id,mine:row.senderUid===id.uid,senderUid:row.senderUid,sender:row.senderName||"",text,time:row.timeLabel||"",state:aggregateReceipt(row,receiptRows.get(row.id),id.uid),cloud:true,e2ee:4,keyEpoch:row.keyEpoch});
