...
for(const required of ["prepareAccountDirectMessage","decryptAccountDirectMessage","m.e2ee===3","recipientKeyId","senderKeyId","kdfVersion"])if(!app.includes(required)&&!fs.readFileSync("firebase.js","utf8").includes(required))throw new Error(`account E2EE v3 runtime anchor missing: ${required}`);
if(!app.includes('text:"",...encrypted'))throw new Error("direct text must persist ciphertext without plaintext leakage");
