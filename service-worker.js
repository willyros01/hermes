...
SHELL.push("./group-message-stream-lifecycle.js","./account-vault-format.js","./account-vault-owner.js","./account-vault-reconciliation.js","./account-vault-activation.js");
const NETWORK_TIMEOUT=4000;

self.addEventListener("install",event=>event.waitUntil((async()=>{await caches.open(CACHE).then(cache=>Promise.allSettled([...SHELL,...FIREBASE_SDK].map(url=>cache.add(url))));await self.skipWaiting();})()));
...
function notificationClickRoute(value){
  try{
    if(!["direct-message","group-message"].includes(value?.type))return null;
    ...
    return{type:value.type,conversationId,messageId};
  }catch{return null;}
}
...
onBackgroundMessage(notificationMessaging,async payload=>{
  ...
  await storePendingNotificationRoute(envelope.route);
