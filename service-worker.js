/*
 * FIDUNIO service worker — 0.9.0 development branch.
 * Network-first application shell; Firebase transport is never SW-cached.
 * The validated 0.8.1.16 receipt fast-path remains applied while 0.9.0
 * per-device E2EE is developed and tested.
 */
...
async function patchedAppResponse(request,response){
  if(!response||!response.ok)return response;
  const url=new URL(request.url);if(url.origin!==self.location.origin||!url.pathname.endsWith("/app.js"))return response;
