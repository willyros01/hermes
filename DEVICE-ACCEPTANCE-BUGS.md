...
#### Device acceptance evidence — startup/PIN pass; E2EE lock identified — 2026-09-07

After two iPhone restarts, the large spinner remained visible until the normal PIN screen appeared; the original PIN unlocked successfully and the Messages screen opened. iPad showed the same bounded startup presentation on its first restart. FDA-IOS-001 and FDA-IOS-002 are therefore **RESOLVED / USER-ACCEPTED** for this candidate. The iPad then displayed the exact fail-closed error `Account E2EE identity is not unlocked for this account.` This establishes that the current send block is local account-E2EE readiness, not proof of failed Firebase connectivity. Queued messages remain preserved. A separate key-change warning is also visible; because the user previously pressed Verify without comparing fingerprints, do not verify the new fingerprint until both devices are deliberately compared. FDA-DM-001 remains OPEN pending proper Account Encryption unlock and Sent → Delivered → Read proof.

#### Account READY retest exposes outgoing-decrypt defect — 2026-09-07

The user supplied iPad and iPhone screenshots showing Account Encryption `READY`. Preserved iPad Outbox rows then reached authoritative `Sent`, proving Firebase accepted the writes, but the iPad rendered its own sent rows as account encryption unavailable. Diagnosis found that runtime used the incoming-only account decrypt direction for every v3 row. The 0.9.9.8 candidate adds one direction-aware reader inside the existing account E2EE service, selected by authenticated UID and authoritative `row.senderUid`, with exact member/keyId validation. It also identifies the changed-key warning as legacy compatibility-device trust, separate from account E2EE. The earlier un-compared trust action is not accepted as verification. Cache revision advances to `001e`. FDA-DM-001 remains OPEN pending full baseline, promotion and real receipts.
