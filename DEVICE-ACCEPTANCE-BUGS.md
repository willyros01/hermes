## FIDUNIO 1.1.54 — group message Reply
...
- **Observed on 1.1.33:** after **Leave Group**, the departed account could remain in the displayed member list and the group remained in both Messages and Groups. **Add Member** could remain indefinitely at “Loading FIDUNIO users…”.
- **Cause:** the membership transaction was correct, but the client group-list owner merged rows without reconciling authoritative absence; overlapping async snapshot/member reads could finish out of order; and member discovery had no bounded failure/retry state.
...
- **Status:** DEPLOYED DEVICE CANDIDATE. The user reported successful pinned deployment on 2026-09-11; `deleteConversationForEveryoneV1` is ACTIVE and the deletion-barrier Firestore rules are deployed. MAIN/PAGES verification and device acceptance remain pending.

## FDA-GROUP-003 — pre-join epoch error after PIN/background return

- **Build/evidence:** After existing-group membership testing, an added/re-added account opened the group after background PIN unlock and saw `Firebase connection problem: Group epoch envelope does not match this account` plus an encrypted-message placeholder.
- **Cause:** PIN did not change the account identity. The group member document stored `historyFrom`, but the central group-message listener queried every retained source row and attempted to unwrap pre-join epochs whose missing envelope was correct by design.
- **1.1.36 correction:** non-admin ordinary message subscriptions begin at authoritative `historyFrom`; admins retain full retained-source access only for explicit history grants. Direct pre-boundary source reads, receipts for inaccessible messages and epochs without the account's envelope are denied by rules. Membership/epoch writes and granted-history copies are unchanged.
- **Acceptance:** on iPhone and iPad, create retained messages before adding a fresh account; add it and confirm no older source, placeholder or connection banner appears. Send/read post-join messages both ways. Background and PIN-unlock the added device and repeat. Remove and re-add it, repeat the boundary, then deliberately grant earlier history and confirm only the granted copies appear readable. Regress group notification routing, receipts, current membership display and one direct conversation.
- **Status:** 1.1.36 DEPLOYED DEVICE CANDIDATE. Exact implementation tree `5ec21d002765f91f091ba9e6b950fd63e02bf3f0` is on `main` as `5a6449c9165228a0f7cf75418f3dbecadc736f99`; all required repository workflows and Pages passed, live Pages serves 1.1.36, and the user reported successful Firestore-rules deployment on 2026-09-12. Device acceptance remains pending. Item 9 is reopened until this matrix passes.

## FDA-GROUP-004 — group operations denied after 1.1.36 rules deployment

- **Build/evidence:** After the user reported successful deployment, an existing iPad group displayed `Firebase connection problem: Missing or insufficient permissions`; the user reported that all group operations lost authority.
- **Cause:** the 1.1.36 epoch read rule rejected legitimate retained epochs without the current UID envelope. The ordinary administrator stream also doubled as the unbounded retained-history source.
- **1.1.37 correction:** ordinary streams for members and administrators start at their member `historyFrom`; retained history uses a separate administrator-only server read; encrypted epoch documents are readable by current members, but a missing per-account envelope remains cryptographically unusable. Direct pre-boundary messages and receipts remain rule-denied.
- **Acceptance:** confirm existing owner/admin/member group opens without a permissions banner; send/read/receipts both ways; add, remove and re-add; leave as non-owner; rename; background/PIN return; explicit earlier-history grant; verify pre-join source remains absent without a grant; repeat on iPhone and iPad.
- **Status:** 1.1.37 DEPLOYMENT CANDIDATE. Exact implementation tree `42498a160b2765243a94e311a2f8bdcf2008c497` is on `main` as `0b2d104d8b94b44144fd7bf6698d95d5d223e91a`; all required implementation workflows and Pages passed, live Pages serves 1.1.37, and corrective `hr.txt` is published. Corrected rules deployment and device acceptance remain pending.

## FDA-GROUP-005 — terminal group listener remains falsely active

- **Evidence:** after rejoin, the new member could send and delete its own message. The owner saw the sent message only after force-close/reopen, and after the deletion the owner confirmed the message was gone. The displayed permission error therefore came from a dead/stale listener, not denied send/delete authority.
- **Cause:** Firestore terminates a listener after a terminal permission error, but `app.js` retained the group ID reuse guard. Foreground activation then treated the dead stream as live. Group errors also shared the global sticky banner.
- **1.1.38 correction:** one generation-bound lifecycle invalidates only terminal group-message stream failures, ignores callbacks from replaced streams, and permits existing foreground/online/reselect/authoritative-group events to reattach. Receipt/decrypt/history/projection failures do not kill the message stream. Error display is scoped to the affected group and clears only after a server-backed snapshot.
