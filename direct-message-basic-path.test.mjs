...
assert.match(app,/prepareAccountDirectMessage\(\{uid:firebaseUser\.uid,peerUid,conversationId:payload\.conversationId,messageId:payload\.messageId,text:payload\.text\}\)/,"the Outbox owner must prepare one account-encrypted envelope");
...
assert.match(worker,/SHELL_REVISION="0\.9\.9\.9s-attachment-stage-diagnostics"/,"the installed PWA must load the encrypted direct-message candidate");
