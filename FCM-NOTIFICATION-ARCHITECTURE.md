...
## FIDUNIO 1.1.18 — iPad dual click-route delivery

Device acceptance proved the 1.1.17 routed URL twice on iPhone, while iPad twice resumed the existing Settings client without applying the query route. The click owner must preserve `openWindow(routed URL)` for the accepted iPhone/cold path and, when `openWindow()` returns a client, send that same validated opaque route to the returned client through the existing `fidunio-notification-route` protocol and focus it. This is dual delivery of one intent, not a second navigation owner: `app.js` remains the only conversation selector and Firestore/E2EE remain message authority. PIN, Settings, receipts, Outbox, lifecycle recovery and payload privacy are unchanged.
