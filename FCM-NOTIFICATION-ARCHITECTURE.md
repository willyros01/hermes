...
## FIDUNIO 1.1.28 — N6 group notification implementation

The agreed N6 boundary is implemented without a second startup, listener, message or render owner. `notifyGroupMessageCreatedV1` observes only accepted `groups/{groupId}/messages/{messageId}` documents, reads the current authoritative `memberUids`, excludes `senderUid`, and fans out data-only notifications to every enabled installation. FCM batches are bounded to 500 tokens. Optional sender-name display continues to use only the authoritative user profile; plaintext, ciphertext, group name, membership and attachment data are excluded.

The worker and installation-local inbox accept `group-message` as a second validated route type. After PIN/auth/hydration, the one activation owner performs a server-authoritative current-membership read, starts the existing group conversation owner, and submits the exact message through that owner's keyed priority queue. The existing group E2EE projection paints before cache/persistence and receipt maintenance; the route is consumed only after the exact row and group-bound composer are mounted. Removed membership and missing messages are consumed as unavailable; transient failures remain pending. Firestore rules, group E2EE, Outbox and receipt authority are unchanged.
