...
FIDUNIO 1.1.52 PORTABLE ACCOUNT RECOVERY VAULT — 2026-09-13: TODO Item 2 advances as a complete-recovery deployed device candidate. Settings → Data can create and restore an encrypted `.fidunio` recovery file that survives browser-origin deletion. Opening requires matching authenticated UID, existing six-digit PIN and established server recovery authority. Import reconciles current direct/group access, deleted messages and Outbox IDs; attempted-but-absent sends remain failed and cannot auto-replay. Activation atomically replaces only app state/history/Outbox and rolls back after interruption or failed reread. Auth sessions, PIN verifier, biometrics, FCM tokens/routes and device identity are not restored. No Firebase/backend/rules change is included. Commit `47d864f4a4dd41b1a6b6fe91bb4bc4f290555505` passed full Rebuild Baseline Security Gate `34761472319`; GitHub Pages run `34761471712` succeeded. Destructive iPhone/iPad site-data-loss testing remains required.
...
FIDUNIO LARGE-ATTACHMENT NETWORK PREFERENCE — 1.1.41: TODO item 6 is implemented as a pre-send browser network-policy boundary with an authoritative 5 MiB threshold. When Settings → Data → Large attachments on Wi-Fi only is enabled, attachments smaller than 5 MiB remain unrestricted; large selections wait while offline or when the browser positively reports cellular/WiMAX, proceed on positively reported Wi-Fi/Ethernet, and also proceed when the browser does not disclose network type (the required Option 2 behavior for iPhone/iPad Safari). A blocked selection displays `Waiting for Wi-Fi`, retains only one in-memory pending File, rejects a second attachment selection, and automatically re-evaluates on online/offline, browser connection-type, foreground, or Data-setting changes. The file is not read with `arrayBuffer()`, encrypted, placed into the existing encrypted Outbox, or uploaded until the initial policy allows it. `attachment-send-service.js`, E2EE, Firebase Storage ownership in `firebase.js`, direct/group publication, receipts, deletion, notifications and PIN ownership are unchanged. Runtime/cache advance to 1.1.41 / `1.1.41-large-attachment-network-policy`. The waiting File is intentionally page-memory-only in this candidate; iOS termination/reload requires reselection, and an upload already started is not paused if the network changes mid-upload. Local validation is green: 88/88 non-emulator regression checks and 193/193 JavaScript/module syntax checks pass; the protected `firebase-config.js` remains at blob `b81026dcc07b7374d1f48d0cb094764ce28319bd`, `config-firestore.js` is absent, and whitespace checks are clean. Remote publication/workflow/Pages verification are still required; real-device acceptance remains pending and TODO item 6 stays open.

FIDUNIO 1.1.40 INVITATION DEVICE ACCEPTANCE — 2026-09-12: The user confirmed the restored invitation is back to its original accepted form. TODO item 8 / the full invitation-letter restoration is DEVICE ACCEPTED / CLOSED. The accepted invitation mutation owner and invitation/install separation remain unchanged.
...
- Current checkpoint version: **1.1.54**
...
## FIDUNIO 1.1.18 — REJECTED and removed — 2026-09-10

1.1.18 attempted to supplement the 1.1.17 routed `openWindow()` click with a route message to the returned window client. Device testing showed no iPad benefit: notification → PIN still returned to Settings twice. The user also found the accepted 1.1.17 iPhone path more stable and better performing. All 1.1.18 runtime and test-expectation code is removed and the visible frontend/cache revision is restored exactly to 1.1.17. The successfully deployed 1.1.17 data-only backend remains unchanged. No backend, Firebase configuration/rules, PIN, Settings, lifecycle/reconnect, Outbox, receipts, E2EE, group, attachment, disappearing-content or message-projection code changed in the restoration.

## FIDUNIO 1.1.19 — one-shot intensive notification diagnostics — 2026-09-10

This diagnostic-only release preserves the exact 1.1.17 notification action and adds one serialized IndexedDB diagnostic ledger shared by the service worker and page. It correlates worker install/activate, full bounded FCM payload, notification display/click, route validation, pre/post `openWindow()` clients, requested/actual URLs, bootstrap/controller state, PIN completion, hydration/Firebase state, pending-route gates, conversation lookup, route selection, render/lifecycle events, subscription projection/cache/persist/render timing and errors. Settings exposes one report page with Refresh, Copy Full Report and Clear and Start Test. Full opaque UIDs/conversation/message IDs are retained. PINs, passwords, private/recovery keys and authentication/FCM tokens are never captured; only presence/length/suffix metadata is allowed for security secrets. No route, backend, Firebase rules/configuration, message, receipt, Outbox or E2EE behavior changes.

**Result:** the iPad worker received the correct data-only payload and displayed the notification, but no FIDUNIO `notificationclick` or `openWindow()` event occurred. The PWA resumed at plain `/hermes/` with no pending route. Its post-PIN direct-chat appearance came from the already-saved matching conversation selection, not notification routing. The message itself loaded authoritatively; approximately four seconds of projection plus repeated subscription passes is a separate performance/lifecycle finding. No further diagnostic release is planned. The bounded repair will replace click-only handoff with an installation-local pending-notification inbox, using direct sole-item consumption and a multiple-item chooser, then remove the diagnostic instrumentation.

## FIDUNIO 1.1.20 — installation-local notification handoff

For supported background notifications, the worker persists the validated opaque direct-message route before showing the OS notification. After the normal PIN/auth and application-ready gates, FIDUNIO consumes that route on the same installation. One pending conversation opens directly; multiple pending conversations show a chooser. The existing routed click URL remains available where iOS dispatches it. No notification contains message text, attachments, email, keys or tokens, and the inbox never becomes message or receipt authority. On the affected iPad, a manual launch following a displayed notification cannot be distinguished from a tap and may open the sole pending conversation. Version 1.1.20 removes all temporary 1.1.19 diagnostic runtime/UI while retaining its conclusions in documentation.

## FIDUNIO 1.1.21 — single activation owner and composer-safe projection candidate

Device evidence proved 1.1.20 could route twice on iPad and then stop, while iPhone failed. Source review found the process-lifetime `notificationInboxLoaded` flag prevented later foreground activations from rereading newly stored routes. Version 1.1.21 removes that stale cache and routes initial hydration, PIN unlock, Firebase-auth readiness, foreground return, online/offline change and worker messages through one coalescing activation owner. The owner decides notification priority before rendering and consumes the local record only after the direct-chat state is persisted and rendered.

The same review confirmed async Firestore, receipt, attachment, peer-name and Outbox callbacks could replace the entire chat DOM while a user typed. One composer owner now retains an in-memory draft per conversation, focus, selection and scroll state. Background projections update only the owned chat/status/sidebar regions and do not replace the textarea. Drafts are cleared on sign-out and are not persisted as a new plaintext store. Firebase backend/rules, notification payload, E2EE, receipts, message authority and Outbox authority are unchanged. Device acceptance remains required.

## FIDUNIO 1.1.22 — cold-route and immediate message projection correction

Device testing rejected 1.1.21 as complete. Background iPhone/iPad launches reached the direct conversation but displayed the new message only after ten seconds or more; a terminated iPhone opened the Messages list; and both devices opened direct conversations at the oldest message. Source review proved three distinct boundaries. A cold activation could inspect a stale local conversation record before the cloud subscription marked it authoritative, reject it, and consume the notification route. Every message snapshot sequentially re-fetched E2EE peer authority and decrypted the complete history, then awaited local cache/state persistence and server Read recovery before repainting. Finally, 1.1.21 persisted viewport position in the per-conversation composer map, so a later intentional entry restored an obsolete top position.

Version 1.1.22 resolves questionable cold-route metadata through a server-authoritative conversation read before rejection and retains the pending route until the exact target composer is mounted. Known immutable e2ee:3 rows reuse their already authenticated plaintext; only new or previously unavailable rows decrypt. The central render owner projects the merged in-memory snapshot before IndexedDB durability and Read-receipt work. Draft/focus/caret remain per-conversation and memory-only, while scroll becomes a one-render transient handoff: background updates preserve the current viewport and intentional conversation/notification entries open at the latest message.

Changed runtime files are limited to `app.js`, one bounded server-read API in `firebase.js`, `version.js`, and the deterministic service-worker cache revision. No worker notification behavior, backend Function, Firestore rule, payload, token registration, PIN/authentication, E2EE format/key, receipt mutation, Outbox, group, attachment, disappearing-content or Settings owner changed. Permanent notification-route/projection and ownership gates cover the new ordering. Device acceptance remains required.

## FIDUNIO 1.1.23 — deterministic activation, latest snapshot and viewport owner

Real-device testing rejects 1.1.22: background routing usually reached the direct chat but message visibility remained inconsistent; terminated iPhone fell to Messages; and both devices alternated between oldest and newest scroll positions. Source review found three release races. An activation reason could be queued while the current promise was releasing and remain without an owner. Re-entry transferred a direct listener but also launched a competing full-history read, while every snapshot joined an unbounded FIFO projection chain. Finally, an entry-to-latest animation frame could be invalidated by a background render that captured the newly mounted pane at scroll position zero.

Version 1.1.23 keeps the same single owners and closes their release boundaries. The activation owner synchronously starts a new serialized drain if a reason exists when its promise releases. The Firebase direct-message owner retains one live listener, transfers callback ownership without `getDocs()`, and keeps only the latest waiting snapshot while one projection is active. The render owner assigns every render a generation and retains a latest-entry viewport intent until the winning frame applies it; stale frames cannot move the current conversation.

Runtime scope remains `app.js`, `firebase.js`, `version.js`, service-worker cache identity, and permanent gates. Touched screens are startup/PIN transition, Messages-to-chat entry, direct message list, active composer/scroll, and iPad direct pane/sidebar. Required regressions are Settings/Profile/User Administration loading, notification chooser, text send/receipts, offline Outbox, group chat, and one attachment. No notification payload/display, Firebase backend/rules/config, PIN/auth, E2EE format/key, receipt mutation, Outbox semantics, group/attachment/disappearing-content authority, or Settings owner changes.

## FIDUNIO 1.1.24 — keyed notification-priority semaphore

Version 1.1.24 addresses the remaining warm-iPad delay without adding another listener or renderer. A pending direct-message notification supplies the exact `(conversationId, messageId)` key to the existing per-conversation delivery owner. That owner performs one exact server read when the live listener has not already projected the row, runs it at the next safe boundary ahead of snapshot maintenance, and merges it non-authoritatively by message ID. The PIN transition says “Opening message…” until this attempt finishes. The pending record is consumed only after the exact row and conversation-bound composer are mounted; a bounded miss shows “Loading new message…” and remains retryable.

Affected UI is limited to notification → PIN → direct chat, the direct-message row projection, newest-message entry, and the notification loading banner. Normal PIN unlock, Messages, Settings/Profile/User Administration, groups, attachments, Outbox, receipts, and disappearing messages must be regression-tested but their authorities and data formats are unchanged. No Firebase Function, Firestore rule, notification payload, E2EE format/key, or protected configuration changes are included.

**Checkpoint status (2026-09-10): DEVICE ACCEPTED.** The user exercised every defined notification scenario at least five times on both iPhone and iPad and reported that all behaved as expected. FIDUNIO 1.1.24 is the documented checkpoint/rollback baseline for subsequent work.

## FIDUNIO 1.1.25 — password-change PIN bridge correction

The Settings password-change path previously passed the entered six-digit FIDUNIO PIN under an ambiguous `pin` property, while the existing E2EE rewrap owner requires explicit `oldPin` and `newPin` properties. The owner therefore rejected every attempt before Firebase password mutation. Version 1.1.25 supplies the same user PIN explicitly for both sides of the password-only rewrap and applies the identical mapping to the Firebase-failure rollback. The Change Password card now owns its own clearly located Current Password field and clears all transient password/PIN inputs after success.

The correction preserves the existing E2EE identity, public key, `keyId`, message history and six-digit PIN. It does not alter password-reset recovery, ordinary PIN/biometric unlock, Firebase rules/configuration, notifications, messages, receipts, Outbox, groups, attachments, disappearing content, Profile saving, or User Administration. A permanent repository gate covers the forward bridge, rollback bridge, form ownership and secret-field cleanup.
...
Version 1.1.28 extends the device-accepted 1.1.24 notification ownership pattern to encrypted groups. The new server trigger derives recipients only from the group's current `memberUids`, excludes the sender, and sends a data-only generic notification to each enabled installation in batches of at most 500. Payloads contain only `group-message`, opaque conversation/message IDs and the bounded notification body. Notification delivery never grants group or history access.

After PIN unlock, the existing activation mutex validates current group access through `firebase.js`, starts the existing group read/decrypt/receipt owner, and gives the exact message ID priority over queued snapshots. The row is decrypted and projected through the existing owner before cache/persistence or receipt maintenance, then the central render owner opens the group at the latest message. Missing or unauthorized targets fail closed; transient failures retain the installation-local route. No new listener, renderer, Firebase initializer or message store is introduced.

Touched screens/functions: notification display/tap, PIN-to-chat transition, pending-notification chooser, group chat message list/loading banner, group list/sidebar metadata, and optional sender-name notification preference. Direct notifications, ordinary PIN unlock, message composing/sending, group membership/history controls, receipts, Outbox, Settings/Profile/User Administration, attachments, disappearing content and Forgot Password require regression testing but retain their authorities. Repository/workflow, Pages, live Function deployment and repeated iPhone/iPad group-device acceptance are required before N6 is closed.
...
## FIDUNIO 1.1.33 — immediate mass-delete convergence

The first direct device test proved both selected messages were physically deleted on sender and receiver, but one sender row disappeared only after the final Firestore snapshot arrived. The backend removes a page sequentially, so the listener may briefly receive an intermediate snapshot containing a later row. Version 1.1.33 adds one conversation-keyed, memory-only projection owner: only IDs already confirmed deleted by the server are suppressed, including from in-flight/intermediate snapshots, until a full server-backed snapshot proves each source absent. Cache/partial absence cannot release the suppression.
...
The runtime advances from 1.1.34 to 1.1.35. This build also includes the previously held local device-acceptance closeouts for mass delete item 4 and group membership item 9. All 84 non-emulator workflow steps and all six Firestore emulator suites, including the new deletion-barrier suite, pass locally. Exact implementation tree `82c6f1636bef2c9e5356ebc0718abc933f0af557` is published on `main` as `ca265dfe2f5bafc87bdba34e5c9fe4afc1968023`; pinned deploy script follow-up is `522be7b3801456e9d49aac4daa7bf1a73f96a976`. The user reported that pinned deployment completed successfully on 2026-09-11: `deleteConversationForEveryoneV1` is ACTIVE and the deletion-barrier Firestore rules are deployed. GitHub workflow/Pages verification and repeated direct/group iPhone/iPad multi-device acceptance remain required.

## FIDUNIO 1.1.36 — group join-time history boundary correction

After the accepted add/remove/leave matrix, a newly added/re-added group account displayed `Group epoch envelope does not match this account` and an encrypted-message placeholder after returning from background through PIN unlock. PIN did not rotate or replace the E2EE identity. The read-side group owner was subscribing to every retained source row even though each member document already stored `historyFrom`; attempting to decrypt a legitimate pre-join epoch then surfaced as a misleading Firebase connection error.

Version 1.1.36 keeps the membership transaction, epoch rotation and explicit history-grant system unchanged. `firebase.js`, the sole Firebase SDK owner, now reads the signed-in member record before opening the group stream. Non-admin subscriptions query only `createdAt >= historyFrom`; group administrators retain the full source query required to build deliberate encrypted history grants. Firestore rules deny direct pre-boundary message reads, receipt mutations for inaccessible sources and epoch reads when the account has no envelope. Explicit active history-grant copies remain readable through their existing separate path.

The installed PWA advances to 1.1.36 with shell revision `1.1.36-group-history-boundary`. The permanent group-conversation gate now requires the bounded query/rules wiring. The group emulator adds rejoin-boundary, bounded-query, inaccessible-epoch and post-rejoin-epoch cases and passes 46/46 assertions. All 84 non-emulator workflow steps and all six Firestore emulator suites pass locally. Exact implementation tree `5ec21d002765f91f091ba9e6b950fd63e02bf3f0` is published on `main` as `5a6449c9165228a0f7cf75418f3dbecadc736f99`; pinned script follow-up is `a3ce9e61927aab8b79c9c5ca63ca140053381392`. Rebuild Baseline `34669079121`, E2EE Rules `34669079149`, Firebase Adapter `34669079128`, Pages `34669186160`, and follow-up baseline `34669187077` completed successfully. Live Pages serves 1.1.36 / `1.1.36-group-history-boundary`. The user reported successful Firestore-rules deployment on 2026-09-12. iPhone/iPad device re-acceptance remains required.

## FIDUNIO 1.1.37 — group authority recovery

The deployed 1.1.36 epoch-envelope guard converted a legitimate absent pre-join envelope into a Firestore permission denial. Because group display, sending, receipts and membership all share the epoch loader, existing group work could broadly surface `Missing or insufficient permissions`. The ordinary administrator chat stream also retained the unbounded query used for deliberate history grants.

Version 1.1.37 bounds every ordinary group stream, including administrators, at the current member document's `historyFrom`. A new administrator-only one-shot server read in `firebase.js` supplies retained ciphertext solely to the explicit history-grant runtime. Epoch documents return to current-member read authority; their encrypted envelopes remain account-bound, so an absent envelope still cannot yield a group key or plaintext. Direct pre-boundary message reads and receipts remain denied. All 85 workflow run steps and all six Firestore emulator suites pass locally; the expanded group matrix passes 48/48. Exact implementation tree `42498a160b2765243a94e311a2f8bdcf2008c497` is on `main` as `0b2d104d8b94b44144fd7bf6698d95d5d223e91a`; corrective rules script `hr.txt` is published in follow-up `f0ed67f05e2debedd927330d20365b20af19362f` and pinned to the implementation. Rebuild Baseline `34671731563`, E2EE Rules `34671731548`, Firebase Adapter `34671731547`, and Pages `34671731357` completed successfully. Live Pages serves 1.1.37 / `1.1.37-group-authority-recovery`. Corrected rules deployment and device acceptance remain required.

## FIDUNIO 1.1.38 — terminal group-stream recovery

Device testing proved 1.1.37 group send and Delete for Everyone were authorized: the missing incoming message and completed deletion both became visible after force-closing the owner app. The failure was a terminated Firestore group-message listener still recorded as active, so same-group activation incorrectly reused a dead stream and its permission banner remained sticky.

Version 1.1.38 adds one generation-bound lifecycle owner around the existing group stream. Only the terminal message-listener error invalidates the stream; receipt, decrypt, history and projection errors remain nonterminal. Foreground, online, reselect and authoritative group-list events can reattach the same group without polling or duplicate listeners. Late callbacks cannot close or project over a replacement, and the group-scoped banner clears only after a server-backed snapshot. Rules, group membership, E2EE, receipts, deletion, Outbox, notifications, backend Functions and protected Firebase configuration are unchanged. Shell revision is `1.1.38-group-stream-recovery`.
FIDUNIO PIN + DIRECT ATTACHMENT FORWARD PROGRESS — 1.1.39: Real-device evidence on 1.1.38 showed two independent lifecycle stalls. Opening a direct-message notification and entering the six-digit PIN could leave iOS permanently at Checking because the local WebCrypto verification promise had no completion boundary. Direct photo encryption/upload could complete and durably queue its message, but a stalled first authoritative Outbox reconciliation left no retry trigger until restart. 1.1.39 bounds only the unlock-screen PIN wait at 12 seconds; timeout/error renders a fresh enabled PIN screen with a truthful Try Again message and does not label the PIN incorrect. After a direct attachment commit attempts the existing authoritative Outbox cycle, the exact message ID is checked; if it remains pending, the app reuses the existing immediate/1.5s/4s reconnect recovery owner. Successful sends schedule nothing. Group attachment/text paths, Storage upload, Firebase auth, notification routing, PIN storage, E2EE, rules, receipts and delete authority are unchanged. Runtime/cache advance to 1.1.39 / `1.1.39-pin-attachment-forward-progress`. Device acceptance is pending.
FIDUNIO FULL INVITATION LETTER RESTORATION — 1.1.40: TODO item 8 restores the exact accepted invitation subject and full letter from the validated 0.9.5.1 checkpoint and safe 0.9.4.10 stabilization branch. Copy Invitation, Email Invitation and Share all use the same restored text: inviter identity, FIDUNIO introduction, assigned role, expiration, clean single-use Join URL, personal/non-forward warning, clean public Quick Start Guide URL, guide summary and FIDUNIO sign-off. The existing invitation owner, Firestore schema/lifecycle, redemption, revocation and install separation remain unchanged; the already-authoritative inviter name written during creation is now returned to the share renderer. Rejected 0.9.4.12–0.9.4.15 automatic invite/install code remains absent. Runtime/cache advance to 1.1.40 / `1.1.40-share-copy-restoration`. Device acceptance is pending.

FIDUNIO 1.1.39 DEVICE ACCEPTANCE — 2026-09-12: The user confirmed notification-to-PIN unlock passed; direct photo sent without restart, advanced through Queued/Sent and arrived on iPhone with some delay; group send, receive and delete regression passed. The 1.1.39 PIN and direct-attachment forward-progress correction is device accepted.
...
## FIDUNIO 1.1.48 — Item 9 earlier-history grant eligibility correction

During final Item 9 device acceptance, add/remove/re-add and join-time history behavior passed, but an owner attempt to grant a re-added member retained history **From beginning** returned Firestore `Missing or insufficient permissions`. The history-grant Firestore authority intentionally permits copies only from current encrypted group messages (`e2ee == 4`). The runtime had been feeding every retained group row into the grant builder, including legacy pre-v4 rows, so a legacy source could reach the stricter grant rule. 1.1.48 filters retained grant sources to `e2ee == 4` / `fidunio-group-message-v1` before boundary selection, encryption, grant creation, and copy writes. If no eligible encrypted history exists for the requested range, the user gets an explicit safe message instead of a permission error. No Firestore rules/backend, membership transaction, epoch rotation, normal group-message visibility, receipts, notifications, PIN, attachments, deletion, or reaction authority changed. Device acceptance of From beginning and selected-date grants remains required before closing Item 9.
