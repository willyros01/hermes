...
1.1.17 device proof passed iPhone notification → PIN → exact direct conversation twice, but iPad resumed its suspended Settings client twice and did not retain the routed query. 1.1.18 preserves the accepted `openWindow(routed URL)` path and additionally posts the same validated opaque route to the client returned by `openWindow()`, then focuses it. Only `service-worker.js`, the N5 gate, the visible version/cache revision and required documentation change. No backend, Firebase configuration/rules, PIN, Settings, lifecycle/reconnect, Outbox, receipts, E2EE, group, attachment, disappearing-content or message-projection code changes. Device acceptance remains required.
