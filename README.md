...
Version 1.1.37 bounds every ordinary group stream, including administrators, at the current member document's `historyFrom`. A new administrator-only one-shot server read in `firebase.js` supplies retained ciphertext solely to the explicit history-grant runtime. Epoch documents return to current-member read authority; their encrypted envelopes remain account-bound, so an absent envelope still cannot yield a group key or plaintext. Direct pre-boundary message reads and receipts remain denied. All 85 workflow run steps and all six Firestore emulator suites pass locally; the expanded group matrix passes 48/48. Exact implementation tree `42498a160b2765243a94e311a2f8bdcf2008c497` is on `main` as `0b2d104d8b94b44144fd7bf6698d95d5d223e91a`; corrective rules script `hr.txt` is pinned to that commit. Workflow/Pages verification, corrected rules deployment and device acceptance remain required.
