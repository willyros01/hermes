...
Async message, receipt, conversation, group, peer-name, attachment and Outbox callbacks may update their authoritative state and request projection, but must not replace an active composer. The composer draft/focus/caret/height resource has one per-conversation UI owner. Scroll/viewport is not durable per-conversation state: it may cross only one owned render of the same currently mounted conversation. Intentional notification, Messages-list or other-conversation entry must establish its own newest-message position. Background projections update only owned message/status/sidebar regions. Any future direct global-render call from an async callback requires explicit architecture review and a permanent draft-preservation test.

A pending notification route may be consumed only after the exact target chat has rendered. Locally restored conversation metadata is not authoritative rejection evidence during cold startup; questionable metadata must be resolved through the central Firebase service owner, and transient failure must retain the route for a later serialized activation.
