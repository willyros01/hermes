...
assert.match(sw,/const client=await self\.clients\.openWindow\(target\)/);
assert.match(sw,/client\.postMessage\(\{type:FIDUNIO_NOTIFICATION_ROUTE_MESSAGE,route\}\)/);
assert.match(sw,/await client\.focus\?\.\(\)/);
assert.match(sw,/import \{FIDUNIO_NOTIFICATION_ROUTE_MESSAGE\} from "\.\/notification-routing\.js"/);
...
assert.match(sw,/SHELL_REVISION="1\.1\.18-dual-click-route"/);
assert.match(version,/version: "1\.1\.18"/);
console.log("FCM N5 dual notification click routing gate passed");
