...
assert.match(sw,/client\.navigate\(notificationRouteUrl\(client\.url,route\)\)/);
assert.match(sw,/await \(routed\|\|client\)\.focus\(\)/);
assert.doesNotMatch(sw,/client\.postMessage\(\{type:"fidunio-notification-route",route\}\)/);
assert.match(sw,/clients\.openWindow\(notificationRouteUrl/);
...
assert.match(sw,/SHELL_REVISION="1\.1\.7-fcm-n5-warm-routing"/);
assert.match(version,/version: "1\.1\.7"/);
