FIDUNIO 1.1.52 PORTABLE ACCOUNT RECOVERY VAULT — 2026-09-13: The user rejected same-device-only checkpointing because browser-origin deletion removes it. Active Item 2 delivery is complete recovery through an encrypted `.fidunio` file. Export/import uses existing UID + six-digit PIN + server recovery authority, preserves the same keyId, reconciles cloud deletion/access and Outbox attempt state before atomic activation, and restores the previous installation after interruption/failure. Device-specific auth, PIN/biometric and FCM state is recreated through existing owners. No backend change. Repository/full deployment and real-device site-data-loss acceptance remain pending.
...
Direct and group list listeners now pass/consume cache and pending-write metadata for authoritative absence. Only a full server-backed omission can trigger participant-device removal and serialized local deletion of that conversation's encrypted history, hidden IDs, drafts, attachment URLs and Outbox rows. All 84 non-emulator workflow steps and all six Firestore emulator suites, including the new deletion-barrier suite, pass locally. Exact implementation tree `82c6f1636bef2c9e5356ebc0718abc933f0af557` is on `main` as `ca265dfe2f5bafc87bdba34e5c9fe4afc1968023`; deploy-script follow-up is `522be7b3801456e9d49aac4daa7bf1a73f96a976`. The user reported successful deployment on 2026-09-11; `deleteConversationForEveryoneV1` is ACTIVE and the deletion-barrier Firestore rules are deployed. GitHub workflow/Pages verification and device acceptance remain pending.

## FIDUNIO 1.1.36 group join-time history boundary

Device evidence after add/remove testing exposed FDA-GROUP-003: a newly added/re-added account was offered retained pre-join source rows, so the group read owner attempted epochs that correctly lacked that account's envelope and reported `Group epoch envelope does not match this account`. Background PIN unlock only re-rendered the error; it did not change E2EE identity.

The correction stays inside the existing read authorities. `firebase.js` reads the current member document and constrains non-admin group message listeners to `createdAt >= historyFrom`; admins retain full source access solely for explicit history-grant construction. Rules deny direct pre-boundary message reads, receipts for inaccessible sources and epoch reads without the account's envelope. Active history-grant copies remain separate. Membership writes, epoch rotation, PIN, direct messaging, Outbox, notifications and conversation actions are unchanged. Runtime/cache are 1.1.36 / `1.1.36-group-history-boundary`; all 84 non-emulator workflow steps and all six Firestore emulator suites pass locally. Exact implementation tree `5ec21d002765f91f091ba9e6b950fd63e02bf3f0` is on `main` as `5a6449c9165228a0f7cf75418f3dbecadc736f99`; pinned script follow-up is `a3ce9e61927aab8b79c9c5ca63ca140053381392`. Rebuild Baseline `34669079121`, Rules `34669079149`, Firebase Adapter `34669079128`, Pages `34669186160`, and follow-up baseline `34669187077` completed SUCCESS. Live Pages serves 1.1.36 / `1.1.36-group-history-boundary`. The user reported the pinned Firestore-rules deployment completed successfully on 2026-09-12. Device re-acceptance remains pending.

## FIDUNIO 1.1.37 group authority recovery

Post-deployment evidence immediately rejected 1.1.36 for group availability: the iPad showed `Missing or insufficient permissions`, and the user reported group operations broadly lost authority. The epoch read rule had made presence of the requesting UID's envelope a Firestore authorization prerequisite. Legitimate retained epochs from before a join do not have that envelope, and the shared group epoch loader sits under display, send, receipt and membership flows.

Version 1.1.37 makes every ordinary member/admin group stream query from its member `historyFrom`. Retained-source access is no longer borrowed from that stream; `firebase.js` exposes a separate administrator-only server read used only by explicit history grants. Epoch documents restore the prior current-member encrypted transport boundary. Missing account envelopes remain cryptographically unusable, while direct pre-boundary source reads and receipts remain rule-denied. All 85 workflow run steps and all six Firestore emulator suites pass locally; the expanded group matrix passes 48/48. Exact implementation tree `42498a160b2765243a94e311a2f8bdcf2008c497` is on `main` as `0b2d104d8b94b44144fd7bf6698d95d5d223e91a`; corrective rules script `hr.txt` is published in follow-up `f0ed67f05e2debedd927330d20365b20af19362f` and pinned to it. Rebuild Baseline `34671731563`, Rules `34671731548`, Firebase Adapter `34671731547`, and Pages `34671731357` completed SUCCESS. Live Pages serves 1.1.37 / `1.1.37-group-authority-recovery`. Corrected rules deployment and device acceptance remain pending.

## FIDUNIO 1.1.38 group-message stream lifecycle recovery

The user confirmed the rejoined member's sent message appeared on the owner only after force-close/reopen, and that Delete for Everyone had actually removed the message despite the permission banner. This proves the remaining defect is client listener lifecycle/error projection, not Firestore authorization or deletion authority. The 1.1.38 candidate invalidates a terminal group-message stream with a generation token, releases the same-group reuse guard, ignores late callbacks, reattaches only through existing foreground/online/reselect/authoritative-group events, scopes its error to that group, and clears it only on a server-backed snapshot. No backend or rules deployment is required.
