...
Version 1.1.37 makes every ordinary member/admin group stream query from its member `historyFrom`. Retained-source access is no longer borrowed from that stream; `firebase.js` exposes a separate administrator-only server read used only by explicit history grants. Epoch documents restore the prior current-member encrypted transport boundary. Missing account envelopes remain cryptographically unusable, while direct pre-boundary source reads and receipts remain rule-denied. All 85 workflow run steps and all six Firestore emulator suites pass locally; the expanded group matrix passes 48/48. Exact implementation tree `42498a160b2765243a94e311a2f8bdcf2008c497` is on `main` as `0b2d104d8b94b44144fd7bf6698d95d5d223e91a`; corrective rules script `hr.txt` is pinned to it. Workflow/Pages verification, corrected rules deployment and device acceptance remain pending.
