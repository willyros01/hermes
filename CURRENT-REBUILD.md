...
This is a frontend-only candidate. No backend, rules, configuration, notification payload/display/click, PIN validation, E2EE schema/key, receipt mutation, Outbox policy, group, attachment, disappearing-content, or Settings-owner change is made. All local non-emulator tests, remaining standalone workflow gates, and all five emulator-backed rule suites pass. Product closure remains pending commit/deployment evidence and repeated real-device acceptance.
