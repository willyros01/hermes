...
const PROTOTYPE_CONVERSATIONS=new Map([
  ["1","Maria Santos"],
  ["2","John Cruz"],
  ["3","Family Group"]
]);
...
async function getOrCreateLocalKey(db){
...
async function decryptState(record,key){
...
function isKnownPrototypeConversation(c){
  if(!c||c.cloud||c.cloudGroup)return false;
  return PROTOTYPE_CONVERSATIONS.get(String(c.id))===String(c.name||"");
}
async function migratePrototypeData(){
  const db=await openDb();
  const key=await getOrCreateLocalKey(db);
  let saved=null;
  try{
    const record=await idbRequest(db.transaction("meta","readonly").objectStore("meta").get(STATE_KEY));
    if(record)saved=await decryptState(record,key);
  }catch(err){
    console.warn("FIDUNIO prototype cleanup could not read prior state",err);
  }

  const next=saved&&typeof saved==="object"?{...saved}:{};
  const conversations=Array.isArray(next.conversations)?next.conversations:[];
  next.conversations=conversations.filter(c=>!isKnownPrototypeConversation(c));
  next.messages={...(next.messages&&typeof next.messages==="object"?next.messages:{})};
  for(const id of PROTOTYPE_CONVERSATIONS.keys())delete next.messages[id];
  if(next.selectedId!=null&&!next.conversations.some(c=>String(c.id)===String(next.selectedId))){
    next.selectedId=next.conversations[0]?.id??null;
  }

  const encrypted=await encryptState(next,key);
  const metaTx=db.transaction("meta","readwrite");
  metaTx.objectStore("meta").put(encrypted,STATE_KEY);
  await txDone(metaTx);
  ...
  try{
    ...
    const store=historyTx.objectStore("history");
    for(const id of PROTOTYPE_CONVERSATIONS.keys())store.delete(id);
    await txDone(historyTx);
  }catch(err){console.warn("FIDUNIO prototype history cleanup skipped",err);}
}

function scrubPrototypeUi(){
  document.querySelectorAll(".direct-contact").forEach(el=>el.remove());
  document.querySelectorAll(".section-title").forEach(el=>{
    if(el.textContent.trim()!=="Sample local contacts")return;
    const next=el.nextElementSibling;
    el.remove();
    if(next?.classList.contains("choice-list")&&!next.children.length)next.remove();
  });
}

const observer=new MutationObserver(()=>scrubPrototypeUi());
