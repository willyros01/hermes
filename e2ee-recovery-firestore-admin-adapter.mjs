...
// Intended for Firebase Admin/Cloud Functions. Client code must never import this module.
// Collections are deliberately unmatched by client Firestore rules and therefore deny client access.
...
export function createRecoveryFirestoreAdminRepositories({ db } = {}) {
  ...
  const identityRepo = Object.freeze({
    async readIdentity(uid) {
      const clean = requireText(uid, "uid");
      const snap = await identityRef(clean).get();
      return dataOf(snap);
    }
  });
