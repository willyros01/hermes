...
function openDb(){return new Promise((resolve,reject)=>{let settled=false;const finish=(fn,value)=>{if(settled)return;settled=true;clearTimeout(timer);fn(value);};const req=indexedDB.open(DB_NAME,DB_VERSION);const timer=setTimeout(()=>finish(reject,new Error("Local PIN storage did not open in time.")),8000);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains("meta"))db.createObjectStore("meta");if(!db.objectStoreNames.contains("outbox"))db.createObjectStore("outbox",{keyPath:"id"});if(!db.objectStoreNames.contains("history"))db.createObjectStore("history");};req.onsuccess=()=>finish(resolve,req.result);req.onerror=()=>finish(reject,req.error);req.onblocked=()=>finish(reject,new Error("Local PIN storage is blocked by another FIDUNIO window."));});}
...
let configReadError="";
async function readStoredConfig(){try{const db=await openDb();const stored=await idbRequest(db.transaction("meta","readonly").objectStore("meta").get(CONFIG_KEY));configAvailable=true;if(stored)return normalizeConfig(stored);}catch(err){configReadError=err?.message||String(err);console.warn("FIDUNIO local security config read failed",err);}try{const legacy=JSON.parse(localStorage.getItem(LEGACY_CONFIG_KEY)||"null");if(legacy){const migrated=normalizeConfig(legacy);await writeStoredConfig(migrated);configAvailable=true;configReadError="";try{localStorage.removeItem(LEGACY_CONFIG_KEY);}catch{}return migrated;}}catch{}return defaultConfig();}
let configCache=await readStoredConfig();
let configWriteQueue=Promise.resolve();
function queueConfigMutation(mutator){const run=async()=>{if(!configAvailable)throw new Error(configReadError||"Local PIN storage is unavailable.");const next=normalizeConfig(configCache);await mutator(next);await writeStoredConfig(next);const db=await openDb();const verify=await idbRequest(db.transaction("meta","readonly").objectStore("meta").get(CONFIG_KEY));if(!verify)throw new Error("Local security settings could not be saved on this device.");configCache=normalizeConfig(verify);return normalizeConfig(configCache);};const result=configWriteQueue.then(run,run);configWriteQueue=result.catch(()=>{});return result;}
...
export function getLocalSecurityStatus(){const cfg=loadConfig();return{available:configAvailable,error:configReadError,hasPin:!!cfg.pin,hasBiometric:!!cfg.biometric?.credentialId,timeoutMs:cfg.timeoutMs};}
...
function validStoredPin(pin){return /^\d{4,12}$/.test(String(pin||""));}
...
export function installInactivityMonitor({isUnlocked,onLock}){if(monitor)return;monitor={isUnlocked,onLock};["pointerdown","keydown","touchstart"].forEach(name=>window.addEventListener(name,noteActivity,{passive:true}));document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="hidden"){hiddenAt=Date.now();if(isUnlocked())onLock("background");return;}if(!isUnlocked())return;activityAt=Date.now();hiddenAt=null;scheduleIdleCheck();});window.addEventListener("pagehide",()=>{hiddenAt=Date.now();if(isUnlocked())onLock("background");});window.addEventListener("pageshow",()=>{if(!isUnlocked())return;scheduleIdleCheck();});scheduleIdleCheck();}
