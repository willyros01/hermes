...
### 1.1.22 bounded correction and ramifications

Device evidence after 1.1.21 separates three defects: terminated iPhone notification activation can fall to Messages; warm/background iPhone/iPad chat entry delays the new row by ten seconds or more; and direct chats open at the oldest row from both notification and ordinary list entry. The correction remains inside the approved activation/render boundary and does not authorize general rogue-code cleanup.

Touched screens: startup/loading and PIN transition only as notification-routing boundaries; Messages list as the cold-launch fallback and normal entry source; direct conversation message list, composer and scroll position; iPad direct-chat pane/sidebar. Settings, Profile and User Administration code is unchanged but remains a required neighboring loading test because notification testing often begins there. Group chat, attachment presentation and the multiple-notification chooser are unchanged but are regression-tested at the shared renderer boundary.

Touched functions/resources: pending route application/finalization; one new server-authoritative conversation read behind `firebase.js`; direct snapshot plaintext reuse and projection order; central background chat projection; ephemeral composer state; transient viewport capture/restore; release/cache gates. `scheduleReconnectRecovery`, Outbox serialization, receipt writer, E2EE formats/keys, Firebase rules/backend, service-worker notification display/click, token registration, Settings lifecycle, group owner, attachment transport and disappearing-content owners are not changed.

Required retest: background and terminated notification -> PIN -> exact direct chat on iPhone/iPad; latest-row placement from notification and Messages list; repeated notification-to-visible-row timing; distinctive active draft/focus/caret while incoming and receipt callbacks project; scrolled-up versus near-bottom behavior; ordinary no-notification resume; chooser; text send exactly once and Sent/Delivered/Read; offline replay; one attachment; iPad portrait/landscape; Settings/Profile/User Administration loading. No timer, reload, second subscription, global busy flag or second renderer is introduced.

### 1.1.23 correction and ramifications

1.1.22 device evidence proves its three repairs were incomplete: terminated iPhone can still fall to Messages, message visibility remains variably delayed, and oldest/newest entry remains nondeterministic. The additional source defects are all inside owners already changed in 1.1.21/1.1.22: activation-promise release, direct-message stream scheduling, and deferred viewport restoration. This does not authorize cleanup of unrelated lifecycle code.

Touched functions: `requestAppActivation`, new `startAppActivationOwner`, `subscribeConversationMessages`, new `queueLatestMessageSnapshot`, `render`, `patchActiveChatProjection`, `restoreChatViewport`, and new latest/scheduled viewport helpers. Touched screens/functionality: startup/PIN notification transition, Messages-list entry, direct-chat rows, direct composer focus/caret/scroll, iPad direct pane/sidebar, background receipt/message projection. Neighboring regression surfaces: ordinary resume, Settings/Profile/User Administration loading, notification chooser, group chat, attachments, receipts and offline Outbox.

Not touched: `scheduleReconnectRecovery` policy, Outbox send/reconciliation semantics, receipt writer, E2EE key/format/crypto owners, notification worker payload/display/click/inbox, Firebase backend/rules/config, PIN/auth, group/attachment/disappearing-content implementations, and Settings lifecycle. Re-test all surfaces in `DEVICE-ACCEPTANCE-BUGS.md`; do not accept intermittent success.

### 1.1.24 priority-semaphore ramifications

Touched code is `applyPendingNotificationRoute`, `notificationRouteHasRendered`, `startAppActivationOwner`, `beginCloudMessageSubscription`, the new partial projection helper, `subscribeConversationMessages`, new `prioritizeConversationMessage`, and new `createDirectMessageDeliveryOwner`; version/cache identity and permanent gates also change. Touched screens/functionality are notification PIN transition text, direct-chat entry, direct-message row projection, newest positioning, loading banner, composer-safe background refresh, and warm iPad listener recovery.

The risk boundary is ordering: a priority row must never become authoritative history, maintenance must not overtake it, and consumption must not precede the exact DOM row. A rogue second listener, direct renderer, startup navigator, or global boolean would reintroduce duplicate rows, stale navigation, disappearing drafts, history purge, or Settings hijack. Tests therefore assert one listener implementation, the unchanged stream owner, keyed dedupe, priority scheduling, non-authoritative merge, exact DOM completion, and normal no-notification bypass. Neighboring functionality to retest is Settings/Profile/User Administration, Messages list, composer/caret/scroll, receipts, Outbox, deletion/disappearing messages, groups, attachments, and the notification chooser.
