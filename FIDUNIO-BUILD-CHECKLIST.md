...
| **0.9.9.8** | **IN PROGRESS — Firebase Storage connectivity/deployment repair discovered during 0.9.9.7 preflight.** Register reviewed Storage rules in deployment config, permanently gate the wiring, then perform real authenticated device attachment proof; no opportunistic feature expansion. | Storage bucket/rules setup verified; deployment-wiring gate and full baseline green; real-device upload/download/authorization/offline/purge defects closed. | IN PROGRESS |
