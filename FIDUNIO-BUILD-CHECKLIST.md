...
| Area | State | Evidence / notes |
|---|---|---|
| Disappearing-message policy/model | IN PROGRESS | 0.9.6.11 adds the user-selected per-message duration contract on top of 0.9.6.10 immutable server-backed first-Read authority. `disappearAfterSeconds` is the resolved immutable duration for a disappearing message; absence means off. Message-schema persistence/rules, physical purge execution, local/offline convergence, attachments and anti-resurrection remain unfinished. |
