...
| Area | State | Evidence / notes |
|---|---|---|
| Disappearing-message policy/model | IN PROGRESS | 0.9.6.12 persists the resolved user-selected `disappearAfterSeconds` as bounded immutable outer metadata for current direct/group sends and carries it through encrypted Outbox retry without changing crypto-envelope formats. Physical purge execution, deletion authority, local/offline convergence, attachments and anti-resurrection remain unfinished. |
