...
// message ID. Grant-only rows are appended, deduplicated by source message ID,
// and the final projection is chronologically deterministic.
export function mergeGroupHistoryProjection(liveRows,grantedRows){
  ...
  for(const row of Array.isArray(liveRows)?liveRows:[]){
    ...
    if(grant&&!row.decryptAvailable){
      merged.push({...row,text:grant.text,createdAt:asDate(grant.createdAt)||asDate(row.createdAt),granted:true,historyGrantId:grant.historyGrantId});
    }else{
      merged.push({...row,createdAt:asDate(row.createdAt)});
    }
