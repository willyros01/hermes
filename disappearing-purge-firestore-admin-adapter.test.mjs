...
await check("group commit atomically removes receipts copies empty grant and source while reconciling retained grant",async()=>{const db=new FakeDb(groupSeed),repo=createDisappearingPurgeFirestoreAdminRepository({db});const state=await repo.readGroupPurgeState({groupId:"g1",messageId:"m1"});const result=await repo.commitGroupPurge({groupId:"g1",messageId:"m1",expectedBasis:state.basis});assert.deepEqual(result,{purged:true,alreadyAbsent:false,traceCount:7,storagePurged:false,storagePrefix:null});assert.deepEqual(db.deleted.sort(),[
...
await check("group commit is idempotent after source absence",async()=>{const db=new FakeDb(groupSeed),repo=createDisappearingPurgeFirestoreAdminRepository({db});const state=await repo.readGroupPurgeState({groupId:"g1",messageId:"m1"});db.rows.delete("groups/g1/messages/m1");const result=await repo.commitGroupPurge({groupId:"g1",messageId:"m1",expectedBasis:state.basis});assert.deepEqual(result,{purged:true,alreadyAbsent:true,traceCount:0,storagePurged:false});assert.equal(db.deleted.length,0);});
...
await check("adapter requires Admin Firestore capability but imports no Firebase SDK",async()=>{assert.throws(()=>createDisappearingPurgeFirestoreAdminRepository({db:{}}),/Admin Firestore/);assert.throws(()=>createDisappearingPurgeFirestoreAdminRepository({db:new FakeDb(directSeed),requireStorage:true}),/Admin Storage bucket/);const source=await import("node:fs/promises").then(fs=>fs.readFile(new URL("./disappearing-purge-firestore-admin-adapter.mjs",import.meta.url),"utf8"));assert.equal(/from\s+["']firebase-admin|from\s+["']firebase\/|initializeApp\(/.test(source),false);});
