...
edit('.github/workflows/rebuild-baseline-security.yml',
     '      - {name: Group sender time, run: node group-sender-time.test.mjs}\n',
     '      - {name: Group sender time, run: node group-sender-time.test.mjs}\n      - {name: Message reactions policy and wiring, run: npm run test:message-reactions}\n')
...
s=read('README.md')
if '- Current checkpoint version: **0.9.9.13**' in s:
    s=s.replace('- Current checkpoint version: **0.9.9.13**','- Current checkpoint version: **1.1.43**',1)
write('README.md',s)
