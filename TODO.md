...
2. **Persistence of FIDUNIO on trusted local devices / encrypted vault reconnection — 1.1.52 COMPLETE-RECOVERY DEVICE CANDIDATE** — the user explicitly rejected a same-device checkpoint as an adequate delivered result because it cannot survive browser-origin deletion. Version 1.1.52 implements a portable encrypted `.fidunio` whole-account recovery file under `ACCOUNT-VAULT-AUTHORITY.md`. Export/import requires the matching authenticated UID, existing six-digit PIN and established server recovery authority; import reconciles current cloud deletion/access plus every Outbox attempt before atomic local activation. No Firebase/backend change is required. Repository/full deployment proof and real iPhone/iPad site-data-loss recovery remain required before closure.
