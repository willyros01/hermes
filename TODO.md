...
3. **Administrator-authorized account recovery — 1.1.45 REPOSITORY CANDIDATE; FUNCTION DEPLOYMENT + DEVICE ACCEPTANCE PENDING** — Settings → User Administration → selected user now includes **Start Account Recovery**. An Owner/Admin may authorize only accounts already within their existing management authority; self-recovery and recovery of the Owner are excluded, and an Admin cannot authorize another Admin. The server creates an exact-UID, single-use authorization valid for **30 minutes**, records initiating administrator, timestamps and status, and stores only a SHA-256 identifier for the opaque recovery token. The administrator may Copy, Email, Share or revoke the authorization but never sees or supplies the user's password, PIN, private encryption key, recovery unlock key, messages or attachments. The dedicated recovery page lets the user send the normal Firebase password-reset email, sign in with the new password and privately enter the existing six-digit FIDUNIO PIN. The existing E2EE recovery core then restores the same account encryption identity/key; roles, memberships and history authority are not changed. Root `ar.txt` is the controlled deployment handoff for only the five new recovery functions. The Admin Recovery Gate is green. Deploy `ar.txt`, then complete the administrator/user iPhone/iPad acceptance matrix before closing Item 3.
...
- `ar.txt` deploys only `createAdminRecoveryAuthorizationV1`, `listAdminRecoveryAuthorizationsV1`, `revokeAdminRecoveryAuthorizationV1`, `startAdminAuthorizedRecoveryV1`, and `completeAdminAuthorizedRecoveryV1`.
- Required closure: successful deployment plus real iPhone/iPad administrator/user acceptance. Item 3 remains open until those checks pass.
...
- Item 3 / administrator-authorized account recovery is the active 1.1.45 repository candidate. Backend function deployment and real-device acceptance remain required.
- Item 5 / delete or archive conversation was explicitly accepted by the user and is DEVICE ACCEPTED / CLOSED on 2026-09-12.
