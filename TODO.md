...
9. **Add or remove members in an existing group conversation — DEVICE ACCEPTED / CLOSED 2026-09-11** — Group Info exposes administrator-only **Add Member** and **Remove** actions through the existing serialized membership/E2EE epoch-rotation authority. Version 1.1.34 corrects the reported post-membership projection failures: stale overlapping group snapshots are discarded, member rows are filtered by authoritative parent `memberUids`, confirmed leave immediately removes the local group/chat projection, and later full server snapshots remove groups that are no longer authorized. Cache-only or pending-write absence cannot remove a group. Add Member discovery has a bounded wait, Cancel, explicit error, and Try Again instead of an endless loading panel. Added members begin at join time unless history is separately granted; removed members receive no future epoch or messages. The user reports every defined item 9 test passed, including the existing-group membership lifecycle and related regressions. Item 9 is device accepted and closed; direct conversations remain exactly two accounts.
...
- FIDUNIO 1.1.34 existing-group membership projection and member discovery are DEVICE ACCEPTED: the user reports all item 9 tests passed. The established membership transaction, key-epoch rotation, rules, and direct two-account boundary remain unchanged.
