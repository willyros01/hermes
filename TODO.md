...
5. **Delete or archive a conversation from the Messages list — IMPLEMENTED + DEPLOYED IN 1.1.35; DEVICE ACCEPTANCE PENDING** — every direct/group row in the Messages list has an accessible Conversation actions button. Archive is installation-local, preserves all shared history, remains archived when new messages arrive, and can be restored from the Archived list. Delete is permanent for every participant: either direct participant may delete a direct chat, while only the group owner may delete a group. A typed `DELETE` confirmation invokes the sole server callable, freezes new writes, removes all attachments before recursively deleting messages, receipts, history grants/copies, epochs, members and the parent conversation/group. Full server-backed list absence removes the row and purges that conversation's encrypted local history and Outbox rows on each participant installation. Client Firestore delete remains denied. The user reported the pinned deployment script completed successfully on 2026-09-11; `deleteConversationForEveryoneV1` is ACTIVE and the deletion-barrier Firestore rules are deployed. Complete direct/group/iPhone/iPad/multi-device acceptance before closing.
6. **Enforce “Large attachments on Wi-Fi only” — IMPLEMENTED IN 1.1.41; DEVICE ACCEPTANCE PENDING** — authoritative threshold is **5 MiB**. With the Data setting enabled, smaller attachments remain unrestricted; a large selection waits while offline or when the browser positively reports cellular/WiMAX, proceeds on positively reported Wi-Fi/Ethernet, and proceeds when network type is undisclosed (Option 2, required for iPhone/iPad Safari). The blocked row shows **Waiting for Wi-Fi**, only one memory-only File may wait, a second selection is rejected, and the pending selection is re-evaluated on network/setting/foreground changes. Policy is checked before the File is read, encrypted, staged into the existing encrypted Outbox, or uploaded. Existing attachment/E2EE/Firebase/direct/group/Outbox/receipt/delete/notification/PIN owners remain unchanged. Known candidate limitation: a reload/terminated PWA loses the pending File and requires reselection; an already-started upload is not paused on a later network change. Keep item 6 open through the full direct/group photo/file/audio/video device matrix and determine whether restart durability needs a follow-up.
...
9. **Add or remove members in an existing group conversation — 1.1.37 AUTHORITY-RECOVERY CANDIDATE; RULES DEPLOYMENT + DEVICE RE-ACCEPTANCE PENDING** — Group Info exposes administrator-only **Add Member** and **Remove** actions through the existing serialized membership/E2EE epoch-rotation authority. Version 1.1.34 corrected the original projection failures. Version 1.1.36 added join-time message filtering, but its deployed epoch-envelope read rule caused FDA-GROUP-004: existing group operations broadly failed with Missing or insufficient permissions. Version 1.1.37 applies `historyFrom` to every ordinary group stream, moves unbounded retained-source access to a separate administrator-only history-grant read, and restores encrypted epoch documents to current-member read authority; missing envelopes remain cryptographically unusable and do not grant plaintext. Direct pre-join message reads and inaccessible receipts remain denied. Deploy the corrected rules and complete new/re-added-member iPhone/iPad acceptance before re-closing item 9. Direct conversations remain exactly two accounts.
...
- FIDUNIO 1.1.34 membership projection/member discovery passed their device matrix, but item 9 remains reopened by FDA-GROUP-003/FDA-GROUP-004 until the 1.1.37 authority-recovery rules are deployed and device re-acceptance passes. The established membership transaction, key-epoch rotation, and direct two-account boundary remain unchanged.
