...
9. **Add or remove members in an existing group conversation — 1.1.36 DEPLOYED DEVICE CANDIDATE; DEVICE RE-ACCEPTANCE PENDING** — Group Info exposes administrator-only **Add Member** and **Remove** actions through the existing serialized membership/E2EE epoch-rotation authority. Version 1.1.34 corrected the reported post-membership projection failures and passed the defined device matrix, but subsequent use exposed FDA-GROUP-003: a newly added/re-added account could receive pre-join source rows, fail their deliberately absent epoch envelopes, and show a misleading Firebase/encryption error after PIN/background return. Version 1.1.36 makes the central group-message subscription query from the member document's authoritative `historyFrom`, retains unbounded source access only for group administrators performing explicit history grants, denies direct pre-join source reads and epoch reads without the account's envelope, and prevents receipts for inaccessible rows. Explicit granted-history copies remain separate. The membership writer and epoch rotation are unchanged. The user reported successful rules deployment on 2026-09-12; complete new/re-added-member iPhone/iPad acceptance before re-closing item 9. Direct conversations remain exactly two accounts.
...
- FIDUNIO 1.1.34 membership projection/member discovery passed their device matrix, but item 9 is reopened by FDA-GROUP-003 until 1.1.36 join-time history-boundary device re-acceptance. The user reported successful rules deployment on 2026-09-12. The established membership transaction, key-epoch rotation, and direct two-account boundary remain unchanged.
