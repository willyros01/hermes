...
2. **Persistence of FIDUNIO on trusted local devices / encrypted vault reconnection — 1.1.52 COMPLETE-RECOVERY DEPLOYED DEVICE CANDIDATE** — the user explicitly rejected a same-device checkpoint as an adequate delivered result because it cannot survive browser-origin deletion. Version 1.1.52 implements a portable encrypted `.fidunio` whole-account recovery file under `ACCOUNT-VAULT-AUTHORITY.md`. Export/import requires the matching authenticated UID, existing six-digit PIN and established server recovery authority; import reconciles current cloud deletion/access plus every Outbox attempt before atomic local activation. No Firebase/backend change is required. Implementation commit `47d864f4a4dd41b1a6b6fe91bb4bc4f290555505`, full Rebuild Baseline Security Gate `34761472319`, and Pages run `34761471712` succeeded. Real iPhone/iPad site-data-loss recovery remains required before closure.
...
1. **Portable encrypted vault — ACTIVE 1.1.52 scope:** allow the user to create one `.fidunio` recovery file containing the encrypted account snapshot plus a securely wrapped Vault Backup Key. It must survive icon removal, origin-data loss or reinstall without exposing usable data to possession of the file alone.
