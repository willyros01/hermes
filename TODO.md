...
6. **Enforce “Large attachments on Wi-Fi only” — 1.1.42 CORRECTION; DEVICE ACCEPTANCE PENDING** — threshold remains **5 MiB**. Real-device iPhone cellular testing exposed that 1.1.41 silently sent a 19 MB attachment because Safari did not disclose network type. That Option 2 fallback is rejected. With the setting enabled, large attachments on positively reported cellular/WiMAX or offline still wait; positively reported Wi-Fi/Ethernet proceeds; unknown/unverifiable network type now fails closed and requires an explicit **Send Anyway** confirmation before any file read, encryption, Outbox staging or upload. Cancellation sends nothing. Existing attachment/E2EE/Firebase/direct/group/Outbox/receipt/delete/notification/PIN owners remain unchanged. Keep item 6 open through repeat direct/group photo/file/audio/video device acceptance.
