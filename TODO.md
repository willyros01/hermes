...
9. **Add or remove members in an existing group conversation — IMPLEMENTED; 1.1.34 DEVICE RETEST PENDING** — Group Info exposes administrator-only **Add Member** and **Remove** actions through the existing serialized membership/E2EE epoch-rotation authority. Version 1.1.34 corrects the reported post-membership projection failures: stale overlapping group snapshots are discarded, member rows are filtered by authoritative parent `memberUids`, confirmed leave immediately removes the local group/chat projection, and later full server snapshots remove groups that are no longer authorized. Cache-only or pending-write absence cannot remove a group. Add Member discovery now has a bounded wait, Cancel, explicit error, and Try Again instead of an endless loading panel. Added members still begin at join time unless history is separately granted; removed members receive no future epoch or messages. Repeat iPhone/iPad owner/admin/member, addition, removal, leave, history, notification, receipt, restart and direct-chat regression tests before closing.
...
- FIDUNIO 1.1.34 corrects existing-group membership projection and member discovery without changing the established membership transaction, key-epoch rotation, rules, or direct two-account boundary. Device retest remains required.
