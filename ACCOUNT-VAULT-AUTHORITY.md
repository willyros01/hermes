...
**STATUS: BINDING RECOVERY CONTRACT — FIDUNIO 1.1.53; DEVICE ACCEPTED / CLOSED ON IPHONE AND IPAD 2026-09-13**

## Device acceptance closeout — 1.1.53

On 2026-09-13 the user completed the destructive portable-vault recovery path on both iPhone and iPad. On each device, a portable `.fidunio` recovery file was created and preserved outside Safari/FIDUNIO, the Home Screen installation and browser website data were removed, FIDUNIO was reinstalled, and the same Firebase account was signed in. Current cloud-authorized message history repopulated from Firestore before vault import. After the 1.1.53 iOS file-picker correction, the preserved `.fidunio` file was selectable, restore completed, and the user confirmed recovery passed.

Accepted device evidence covers recovery of current direct/group history and normal post-restore operation, including messaging/receipt behavior, attachment access, Settings review, and six-digit PIN/background return. Installation-local notification/biometric state remains intentionally recreated through its existing owners rather than restored from the vault.

This closes TODO Item 2 for the required iPhone/iPad recovery matrix. Fire 8 behavior remains a platform follow-up only if separately activated; it is not a blocker for the accepted iPhone/iPad closeout.
...
Permanent wiring coverage must reject reintroduction of a restrictive `accept` filter on `restoreVaultFile`. Real-device restore/import acceptance passed on both iPhone and iPad on 2026-09-13.
...
The required destructive device acceptance matrix passed on iPhone and iPad on 2026-09-13: create and preserve a `.fidunio` file, remove Home Screen/browser site data, reinstall, sign into the same account, restore/import with the existing PIN/server authority, and verify normal recovered operation. Notification/biometric capabilities continue to be recreated through their normal owners rather than restored from the vault.
